...
public class ResultAnalyser {
	...
	public static void analyseProbabilityDistribution(
			ParametersContainer parameterContainer, List<ImageDTO> testImageList,
			List<ImageDTO> trainImageList, String baseProbabilityImagePath) {
						...
						for (int label = 0; label < Constants.NUMBER_OF_STATES; label++) {
							...
							if (feature instanceof FeatureContainer) {
								...
								for (Feature singleFeature : featureContainer.getFeatures()) {
									...
									if (!hasAllZeros) {
										double probabilityFeatureLabel = CRFUtils.getFeatureOnLabelProbability(null, currentImage, label, singleFeature, trainImageList, currentProbabilityEstimator);
								...
								if (currentprobabilityEstimator != null) {
									double probabilityFeatureLabel = CRFUtils.getFeatureOnLabelProbability(null, currentImage, label, feature, trainImageList, currentprobabilityEstimator);
									currentFeatureOnLabelConditionalProbability *= probabilityFeatureLabel;
									isProbabilityZero = false;
								}
							}
