\documentclass[12pt]{report}
\usepackage[subpreambles=true]{standalone}
\usepackage[utf8]{inputenc}
\usepackage[english, polish]{babel}
\let\lll\undefined
\usepackage{import}
\usepackage[table,xcdraw]{xcolor}
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{graphicx}
\usepackage{amsmath,amssymb,amsfonts}
\usepackage{txfonts}
\usepackage{pdfpages}
\usepackage{afterpage}
\usepackage{lipsum}
\pagestyle{headings}
\usepackage{mathptmx}
\usepackage[font=small,labelfont={bf,it},textfont={it},justification=centering,skip=2pt]{caption}
\usepackage[backend=biber,sorting=none,style=numeric,giveninits=true,doi=false,url=false]{biblatex}
\usepackage[autostyle]{csquotes}
\usepackage{nameref}
\usepackage[parfill]{parskip}
\usepackage{url}
\usepackage{breakurl}
\usepackage[breaklinks]{hyperref}
\def\UrlBreaks{\do\/\do-}
\usepackage{subcaption}
\usepackage{textcomp}
\usepackage{multicol}
\usepackage{multirow}
\usepackage{hyperref}
\usepackage{siunitx}
\usepackage{booktabs} 
\usepackage{numprint}
\usepackage{makecell}
\usepackage{array}
\usepackage{graphbox}
\usepackage[section]{placeins}
\usepackage[super]{nth}
\usepackage[nottoc,numbib]{tocbibind}
\usepackage{tocloft}
\hypersetup{
    colorlinks,
    citecolor=black,
    filecolor=black,
    linkcolor=black,
    urlcolor=black
}

\addbibresource{bibliography.bib}
\addbibresource{website_bibliography.bib}

\newcommand\blankpage{%
    \null
    \thispagestyle{empty}%
    \addtocounter{page}{-1}%
    \newpage}
\newcommand*{\captionsource}[3]{%
  \caption[{#1}]{%
    #2%
    \\\textbf{Source:} #3
  }%
}


\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\argmin}{arg\,min}
\DeclareMathAlphabet{\pazocal}{OMS}{zplm}{m}{n}
\DeclareQuoteStyle{polish}%
  {\textquoteleft}
  {\textquoteright}
  [0.05em]
  {\textquotedblleft}
  {\textquotedblright}

\setlength{\textwidth}{14cm}
\setlength{\textheight}{20cm}
\setlength{\parskip}{1em}
\renewcommand\theadfont{\itshape}
\renewcommand\theadgape{\Gape[4pt]}
\renewcommand{\baselinestretch}{1.5}
\renewcommand*\cftfigpresnum{\bfseries Figure ~}
\settowidth{\cftfignumwidth}{\cftfigpresnum}
\renewcommand{\cftfigaftersnumb}{\quad:~}
\renewcommand*\cfttabpresnum{\bfseries Table~}
\settowidth{\cfttabnumwidth}{\cftfigpresnum}
\renewcommand{\cfttabaftersnumb}{\quad:~}
\graphicspath{ {./images/} }

\PassOptionsToPackage{export}{adjustbox}
 
\fboxsep=0mm%padding thickness
\fboxrule=1pt%border thickness

\title{Semantic image segmentation with use of conditional random fields}
\author{Aneta Andrzejewska}
\date{18 February 2019}

\begin{document}

\thispagestyle{empty}%
\addtocounter{page}{-1}%
\begin{titlepage}
\afterpage{\blankpage}
\includepdf[pages=-]{title_page_mgr.pdf}
\end{titlepage}


\thispagestyle{empty}
\afterpage{\blankpage}
\import{sections/}{abstract}
\thispagestyle{empty}
\afterpage{\blankpage}
\addtocounter{page}{-1}%
\includepdf[pages=-]{acknowledgements.pdf}
%\newpage

\tableofcontents
\newpage

\chapter{Introduction}
    \import{sections/introduction/}{introduction}
    
    \section{Purpose and scope of the thesis}
    \import{sections/introduction/}{purpose}
    
    \section{Goals of the thesis}
    \import{sections/introduction/}{goal}
    
    \section{Thesis contents}
    \import{sections/introduction/}{contents}

\chapter{Image Segmentation}
    \label{chapter:segmentation}
    \import{sections/image_segmentation/}{introduction}
    
    \section{Semantic Image Segmentation}
    \import{sections/image_segmentation/}{semantic_segmentation}
    
    \section{Semantic Image Segmentation methods}
    \import{sections/image_segmentation/}{semantic_segmentation_methods}

\chapter{Structured Prediction}
    \label{chapter:structured_prediction}
    \import{sections/structured_prediction/}{introduction}
    
    \section{Probabilistic Graphical Model}
    \import{sections/structured_prediction/}{pgm}
    
    \section{Factorisation}
    \import{sections/structured_prediction/}{factorisation}
    
    \section{Conditional Random Fields }
    \import{sections/structured_prediction/}{crf}
    	
    \section{Inference in factor graphs}
    \label{sec:inference}
    \import{sections/structured_prediction/}{inference}
    
    \section{Parameter training }
    \import{sections/structured_prediction/}{parameter_training}
    
    \section{Energy formulation}	
    \label{sec:energy}
    \import{sections/structured_prediction/}{energy}
    

\chapter{Semantic image segmentation system}
    \label{chapter:system}
    \import{sections/system/}{introduction}
    
    \section{System goals}	
    \import{sections/system/}{goals}
    
    \section{System components}	
    \import{sections/system/}{components}
    
    \section{Technological stack}	
    \import{sections/system/}{stack}

\chapter{Colour-based semantic image segmentation}
    \label{chapter:linear}
    \import{sections/practical_crf_linear/}{introduction}
    
    \section{Preprocessing}
    \label{sec:linear_preprocessing}
    \import{sections/practical_crf_linear/}{preprocessing}
    
    \section{Feature function definition}	
    \import{sections/practical_crf_linear/}{feature_function}
    
    \section{Dataset}	
    \label{sec:linear_dataset}
    \import{sections/practical_crf_linear/}{dataset}
    
    \section{Evaluation method}	
    \import{sections/practical_crf_linear/}{evaluation_method}
    
    \section{Experimental results}	
    \import{sections/practical_crf_linear/}{experiments}
    



\chapter{Shape-based semantic image segmentation}
    \label{chapter:nonlinear}
    \import{sections/practical_crf_nonlinear/}{introduction}
    
    \section{Feature function definition}
    \import{sections/practical_crf_nonlinear/}{feature_function}
    
        \subsection{Feature function for the unary potential}
        \label{sec:nonlinear_unary_potential}
        \import{sections/practical_crf_nonlinear/}{unary_potential}
        
        \subsection{Probability distribution estimation}
        \import{sections/practical_crf_nonlinear/}{pdm_estimation}
        
        \subsection{Feature function for the pairwise potential}
        \import{sections/practical_crf_nonlinear/}{pairwise_potential}
        
    \section{Semantic segmentation on noise free images}
    \import{sections/practical_crf_nonlinear/noise_free/}{introduction} 
        
        \subsection{Dataset}
        \import{sections/practical_crf_nonlinear/noise_free/}{dataset}
     
        \subsection{Feature selection results}
        \label{sec:feature_selection_noise_free}
        \import{sections/practical_crf_nonlinear/noise_free/}{feature_selection}
        
        \subsection{Experimental results}	
            \import{sections/practical_crf_nonlinear/noise_free/}{experiments}
    
    \section{Semantic segmentation on noised images}
    \import{sections/practical_crf_nonlinear/noised/}{introduction} 
         \subsection{Dataset}
        \import{sections/practical_crf_nonlinear/noised/}{dataset}
     
        \subsection{Feature selection results}
        \label{sec:feature_selection_noise}
        \import{sections/practical_crf_nonlinear/noised/}{feature_selection}
        
        \subsection{Experimental results}	
        \import{sections/practical_crf_nonlinear/noised/}{experiments}

\chapter{Discussion and conclusions}        
    \label{chapter:conclusions}
    \import{sections/conclusions/}{introduction}
    
    \section{Thesis summary}
    \import{sections/conclusions/}{summary}
    
    \section{Improvements perspective}
    \import{sections/conclusions/}{improvements}
   



\newpage
\emergencystretch=2em
\Urlmuskip=0mu plus 1mu\relax

\printbibliography[heading=bibintoc]
\newpage
\listoffigures
\newpage
\listoftables
\newpage

\end{document}
