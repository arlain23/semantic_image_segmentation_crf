...
Next step after feature selection concerns a feature function definition. Initially, the same feature function was used as in \textit{chapter: \ref{chapter:linear}: \nameref{chapter:linear}}, however, the results were poor. Not only did the method fail to distinguish between red objects of different shapes, but it was also not capable of assigning the proper class to all blue regions. Figure \ref{fig:linear_as_nonlinear} presents the results of semantic segmentation on four sample test images. 
\begin{figure}[ht]
